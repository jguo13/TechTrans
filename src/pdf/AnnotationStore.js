...
export default class AnnotationStore {
  ...
  async createAnnotation(annotation) {
    try {
      ...
      console.log(user)
      console.log("135" + JSON.stringify(annotation, null, 2))
      ...
      if (annotation.target.selector !== undefined) {
        ...
        console.log('error46' + JSON.stringify(annotation, null, 2))
        annotation.body.creator = {
          name: user.email,
          id: user.email
        };
      }
      ...
      console.log('error' + JSON.stringify(annotation, null, 2))
  ...
  async getAnnotations(pageNumber) {
    ...
    try {
      ...
      const querySnapshotRelations = await getDocs(
        query(annotationsCollection, where('pageNumber', '==', null))
      );

      const relationsAll = [];
      querySnapshotRelations.forEach((doc) => {
        console.log("in the query");
        // Extract data from the Firestore document
        const annotationData = doc.data();
        for (const field in annotationData) {
